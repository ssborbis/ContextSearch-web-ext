...
document.addEventListener('keydown', (ev) => {
	...
	if (
		...
		((ev.target.type === 'text' || ev.target.type === 'textarea' || ev.target.contentEditable) && !userOptions.quickMenuAutoOnInputs)
	) return false;
...
document.addEventListener('mouseup', (ev) => {
	...
	if (Date.now() - quickMenuObject.lastSelectTime > 1000 && ev.target.type !== 'text' && ev.target.type !== 'textarea'  && !ev.target.contentEditable) return false;
